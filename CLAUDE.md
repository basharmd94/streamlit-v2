...
- **Discount (Order Total)** and **Total Order Amount** are both computed the same way — grouped by `voucher` **across every product on that order**, not just the selected one, via a single `_oa_data.groupby("voucher").agg(...)` pass (`SUM(proddiscount)` and `SUM(final_sales)` respectively).
- **Final Line Amount** = `final_sales` (= `altsales − proddiscount`, already computed by `common.data_copy_add_columns`) — **deliberately not `totalsales`/`opddt.xlineamt`**. For **100005 (Zepto) specifically**, `xlineamt` is wrong: confirmed against a real ERP data pull (order `DO--058487`) that `xlineamt = xdtwotax − xdtdisc − xdisval`, where `xdisval` is the line's share of the item-wise **MRP Discount** (GL `07080001`, `xdtcomm_line × xqty`) — but that MRP gap is *already* implicitly priced into `xdtwotax` (wholesale rate) vs `xprice` (MRP), so subtracting `xdisval` again double-counts it. True per-order revenue is `opdor.xappamt` (= `xdtwotax − xdtdisc`, no `xdisval` term), confirmed to equal Accounts Receivable in `gldetail` (acct `1030001`) to the cent; `opdor.xtotamt` (= `SUM(xlineamt)`) does **not** match AR and should not be used for 100005. `final_sales` reconciles to AR exactly and is mathematically identical to `totalsales` for every other ZID (verified against 100001), so this is a safe universal formula, not a ZID-conditional branch. On real local 100005 data, free-goods lines (100% `xdisc`) previously showed a negative `totalsales` (e.g. `-23`, `-32`, `-34`) under the old formula; `final_sales` correctly nets these to `0`.
