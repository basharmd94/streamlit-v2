...
**Honour Discount characterization (`xdisc` between 0–100%, applied directly on paid lines rather than via a separate free-qty line — a distinct discount lever from free goods, though both post to the same GL `07080002`)** — discussed but **not yet built**: proposed approach is to identify honour-discount lines (`0 < proddiscount < altsales*0.99`, i.e. excluded from the free-goods proxy above), compute the effective `%` per line, and correlate it against order value / customer / salesman to see whether discounting already follows an implicit value-based tiering. Revenue-neutral quantity-tier simulation (apply candidate tier prices to historical total qty, compare to actual `final_sales`) is the planned next phase after both discount levers are characterized — not built yet.
