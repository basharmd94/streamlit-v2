...
Campaign audience builder, built in front of the same WhatsFly template picker used by the single-message panel. Page order is **audience filters first, template picker second**, per explicit ask ("build who, then pick what"). The template picker dispatches to a per-campaign handler registered in `_WF_BULK_TEMPLATE_VIEWS` — empty today, falls back to `_wf_bulk_default_view` (a plain preview) — one handler gets added per real campaign as it's defined, not a generic system built in advance.
...
**Not yet built:**
- **The actual send step** — filtered audience × picked template → dispatch to WhatsFly. Explicitly deferred to its own follow-up request; this feature currently stops at "here's your audience and your template," no send button wired between them yet.
