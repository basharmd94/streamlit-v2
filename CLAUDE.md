...
Single-message test panel, per `Whatsfly_Integration_docs/whatsfly-integration-guide.md` (the build reference this was built against — companion `.html` is the same content, styled for human reading). **Current build phase only**: send one message to one hand-entered number using an already-approved template (or plain session text) and look at the raw API response. **No receive-side / webhook handling in this app** — that's a separate always-on FastAPI service the guide describes as a later phase; this Streamlit panel is pure send-side plumbing, per what was explicitly asked for ("No return no webhooks right now").

- **Credentials**: `config/whatsfly.ini` (gitignored — same convention as every other `*.ini` in `config/`, matches `.gitignore`'s blanket `*.ini` rule), section `[whatsfly]` with `api_token` and `phone_number_id`. `config/settings.py::get_whatsfly_params()` returns `None` (never raises) if the file/section/keys are missing, so `views/marketing.py::_show_whatsfly_messaging` can show a friendly `st.warning` with the exact ini template to create, instead of crashing the page. Not committed anywhere — set it up locally/on-server the same way `global_db.ini` is.
- **`core/whatsfly.py`** — thin client: `send_text` (`POST /whatsapp/send`, session message, needs a 24h-open window), `get_templates` (`POST /whatsapp/get/template/list`), `send_template` (generic — see below). All raise `requests` exceptions up to the view, which catches and `st.error`s them rather than crashing (this is the exploratory "see what feedback we get" phase, not a hardened wrapper).
- **No fixed "send template" endpoint exists** — the guide is explicit that WhatsFly generates a per-template send path from the dashboard's template picker rather than publishing one documented route. `send_template(phone_number, endpoint, extra_params)` therefore takes the endpoint as an argument instead of hardcoding one; the UI pre-fills a best-guess default (`/whatsapp/send/template`) in an editable text input, with a caption pointing at the real dashboard picker as the source of truth if the default 404s/errors.
- **Template list response shape is unconfirmed** against the live account, so the UI never assumes one fixed schema: `_wf_normalize_templates` tries common wrapper keys (`data`/`templates`/`result`/`results`/`list`, a bare list, or a single-template dict) and `_wf_extract_body_text` tries common body-text keys, then Meta Cloud API's own `components: [{type: BODY, text: ...}]` shape, then falls back to a recursive scan for any string containing `{{`. Every response is also shown raw in an `st.expander` (list-level and selected-template-level) so a wrong guess is visible immediately, not silent — verified against three plausible shapes (a `{data: [...]}` wrapper with Meta-style `components`, a bare list with a flat `text` key including a non-English body, and an empty/unrecognized shape) before shipping; all three parse or fail visibly as intended.
- **Variable count is detected, not asked for** — `_wf_variable_count` regexes the extracted body text for `{{n}}` placeholders and renders that many `st.text_input`s (max `n` found, so `{{1}}`+`{{2}}` → 2 inputs even if found out of order).
- **Payload JSON is editable, not just computed** — `_render_wf_template_send` builds a default `{template_name, language_code, variables}` payload (matching this same API's own `custom_fields`-as-JSON convention documented elsewhere in the guide) into an `st.text_area`, seeded via `session_state` only on first render *per template selection* (`payload_key = f"wf_payload_json_{idx}"`) so free-form edits to the JSON persist across reruns instead of being silently overwritten — an explicit **"↻ Rebuild payload from fields above"** button (not automatic re-sync) resets it from the current template-name/language/variable inputs via `st.session_state[...] + st.rerun()`, since auto-syncing on every keystroke would fight a user mid-edit of the raw JSON.
...
Once the account was actually wired up, plain-text session sends turned out to be a dead end for testing (24h-window rule per the guide), so the template path became the primary flow. `_wf_templates_table` renders the "what can we actually access" check that was asked for — one row per template, always visible (not buried in the raw-JSON expander).

**Real response shape confirmed against the live account** (first real send-side bug this feature hit): the template list sits under `{"status": "1", "message": [...]}` — **`message`, not `data`/`templates`** — a genuinely surprising key name (payload array, not an error string) that none of the original guessed wrapper keys covered, so `_wf_normalize_templates` returned `[]` and the panel showed "No templates found" even though the API call itself was succeeding. Fixed by adding `"message"` to the wrapper-key list (kept first, the others stay as fallbacks for wherever this account's shape doesn't match documentation/other endpoints). Confirmed real per-template fields: `id` (WhatsFly's own internal row id), `template_id` (a long numeric Meta-style id — a **different** id from `id`, both real, both shown as separate table columns now rather than collapsed into one guessed "Template ID"), `whatsapp_business_id`, `template_name`, `template_type`, `template_category`. `language`/`status` are kept as guessed fallback columns (the API reference describes an approval `status` on templates that this particular account's response didn't happen to include) — not everything guessed before turned out to exist, and that's fine, the "—" placeholder handles it. Body/header/footer text fields are still unconfirmed against this account (not visible in what was checked so far) — `_wf_extract_components`'s recursive `{{`-scan fallback is what's actually load-bearing here now, and the panel shows "No body text found... check the raw JSON" rather than crashing if a real template's body doesn't surface.
...
First real send attempt against a real template returned `{"status":"0","message":"Message template not found"}` at HTTP 200 — the endpoint itself responds, WhatsFly's own lookup just doesn't match whatever identifier the send call carries, even though that same template was correctly listed a moment earlier.

**Confirmed there is no published contract for this endpoint** — re-checked both `whatsfly-integration-guide.md`/`.html` word-for-word (identical on this point: *"The endpoint you were shown is generated per-template from a picker in the dashboard rather than documented as a fixed path/param set"*), then independently searched and read WhatsFly's own public docs site (`app.whatsfly.net/docs`) — Integration, Webhook Workflow, and Broadcasting pages — which is entirely dashboard/UI documentation (Bot Manager, Broadcast, LiveChat, Webhooks) with no REST API reference section at all. This isn't a guess that was missed; it's a genuine, confirmed gap in what WhatsFly publishes anywhere.

**Two credible payload shapes, user-toggleable, since neither is confirmed:** `_render_wf_template_send` now has a `payload_shape` radio (`f"wf_payload_shape_{idx}"`) —
- **"Flat (template_id/template_name)"** — the original guess, both of the template-list's confirmed real identifiers (`id` vs. the longer `template_id`) sent alongside `language_code`/`variables`, since it wasn't known which one (if either) send keys off.
- **"Meta Cloud API style (nested template/components)"** (new default) — WhatsFly's own Overview says it sits on top of Meta's *official* WhatsApp Cloud API, which has a real, stable, publicly documented template-send shape (`template: {name, language: {code}, components: [{type: "body", parameters: [{type: "text", text: ...}]}]}`) — a BSP wrapper passing this shape through largely as-is is a well-grounded bet, not an arbitrary third guess.

`payload_key` is now `f"wf_payload_json_{idx}_{shape_key}"` (shape folded in) so switching the radio shows that shape's own fresh default (or whatever was left there from a prior edit) instead of stale JSON from the other shape.

**Real bug this surfaced and fixed**: `core/whatsfly.py::send_template` always sent via `requests.post(data=payload)` (form-encoding) — fine for the flat shape, but a nested dict/list value (the Meta-style `template` object) can't be form-encoded directly; `requests` would silently mangle it into a Python-repr string instead of JSON. Fixed by JSON-stringifying any dict/list value in the merged payload before the request goes out — matching this same API's own established convention elsewhere (`assign-custom-fields`' `custom_fields` is documented as a JSON-encoded string in a flat form field), rather than guessing between `requests`' `data=` and `json=` bodies. Verified via a mocked `requests.post` that the nested Meta-style payload now arrives as one clean JSON string per field, and the flat shape is unaffected (no dict/list values to touch).
...
**Next real error hit, once the shape was right**: `{"status":"0","message":"Unsupported post request. Object with ID '<phone_number_id>' does not exist, cannot be loaded due to missing permissions, or does not support this operation..."}` — this is a **Meta Graph API-level** rejection, not a WhatsFly-level "template not found" anymore, which is itself confirmation the request shape is now correct: WhatsFly accepted it and proxied straight through to Meta, and Meta rejected the configured `phone_number_id` specifically. `_render_wf_response` now pattern-matches `"does not exist"` + `"graph-api"` in the error message and surfaces a targeted `st.info` pointing at WhatsFly dashboard/Meta Business Manager permissions rather than the request shape — there's nothing left to fix in this Streamlit panel for that specific error; it's an account/WABA-configuration issue on WhatsFly's or Meta's side. This exact message is Meta's generic Graph API catch-all (not a WhatsApp-Cloud-API-specific documented code — checked both `developers.facebook.com/docs/whatsapp/cloud-api/support/error-codes` and the general Graph API error-handling guide, neither lists this phrasing verbatim), so the real causes are the usual Graph-API ones: invalid/deleted phone number ID, an access token without permission on that WABA, incomplete Business Verification, or the integration partner having been disconnected in Meta Business Manager.

### Image header support (`_render_wf_template_send`)

Added once the user created a template with an image header to test. **Not auto-detected** — this account's template-list response doesn't expose header format (confirmed: `_wf_extract_components`'s HEADER-component check only ever finds `text`, never a `format`/`IMAGE` field on this account's templates) — so it's a manual `st.checkbox("🖼️ This template's header is an image")`, off by default, gating a `st.file_uploader` (JPEG/PNG only, matching Meta's real header-image constraints) plus a manual-URL fallback text input.

- **`core/whatsfly.py::upload_media`** — new client function for `POST /whatsapp/upload/media` (multipart, field name literally `media_file` per the guide). Response shape is unconfirmed against this account (first real use of this endpoint) — `_wf_extract_media_ref` unwraps a possible `message`/`data`/`result` wrapper (this account already confirmed `"message"` as its wrapper key for the template list, so that's checked) then guesses `id`/`media_id`/`mediaId`/`media_hash` and `url`/`media_url`/`link`/`file_url`, same defensive multi-key pattern as everything else in this feature. The raw response is always shown in an expander so a wrong guess is visible, not silent.
- **Upload is automatic, not button-gated** — triggers as soon as a file is selected, keyed by `(filename, size)` in `st.session_state` so it doesn't re-upload on every unrelated rerun (e.g. typing in a variable input elsewhere on the page); `st.image(uploaded_file, width=200)` shows an immediate local thumbnail regardless of upload success, so the user gets visual confirmation even before the network call resolves.
- `header_image_param` prefers the uploaded media's `id` over `link`/manual URL when both are available — used only by the Meta Cloud API style shape (see below); the flat shape uses the resolved URL directly under its own confirmed field name.

**Flat-shape contract fully confirmed via TWO real dashboard-generated examples now** (text-only, then an image-header + named-variable template — `whatsapp_support_contact`) — two guessing rounds were wrong before the second real example settled it:

1. First guess (`media_id`/`media_url`, no type) → `{"status":"0","message":"header: Format mismatch, expected IMAGE, received UNKNOWN"}`.
2. Re-read the guide, found `send/interactive-buttons`/`send/file` both document a `media_url`/`media_id` **+ `media_type`** trio, added `media_type: "image"` → **same error again**, so that guess was wrong too, not just incomplete.
3. Meta Cloud API style (real, correctly-nested `template.components`, no flat `template_id`) → `{"status":"0","message":"Message template not found."}` — WhatsFly's own not-found message. Confirms the send endpoint's template lookup is keyed on the flat `template_id` field specifically, independent of whatever's nested elsewhere.
4. **User pulled the real dashboard example for this specific template** — the actual confirmed contract is neither of the above:
   - Header image: **`template_header_media_url`** — a plain hosted URL. Not `media_id`, not `media_url`, no `media_type` field at all.
   - Variables: **`templateVariable-<NAME>-<n>`** per variable — e.g. `templateVariable-CUSNAME-1`, `templateVariable-CUSCODE-2` — keyed by the template's own **named** placeholder label (confirmed against the template's stored Meta definition, which shows `example.body_text: [["CUSNAME","CUSCODE"]]` for this template), **not** a numbered/generic `variables` array. Different templates almost certainly use different names, so this can't be hardcoded.
   - `template_name`/`language_code` are absent from both real examples — dropped from the flat default entirely (previously kept as "unconfirmed but harmless" — no longer even included, since two-for-two real examples never showed them).

**UI changed accordingly**: the per-variable input is now two fields (`Name` + `Value`, not just `Value`) — the name is NOT auto-detectable from anything this account's `get/template/list` response returns (no `components`/`example` fields, per the "body/header/footer text fields... unconfirmed" note above). `_WF_DEFAULT_VAR_NAMES = ["CUSNAME", "CUSCODE"]` pre-fills `{{1}}`/`{{2}}`'s Name field — per explicit instruction, since both real dashboard examples so far used exactly this pair in this order, confirmed as this account's actual convention rather than a per-template one-off. A 3rd+ variable has no confirmed name yet and stays blank; still fully overridable per-send if a future template uses different names. `default_payload` for the flat shape is now built as `{"template_id": ..., "templateVariable-<name>-<n>": value, ..., "template_header_media_url": ...}` — nothing else. The Meta Cloud API style shape is kept only as a documented fallback (its `template_name`/`language_code` inputs moved to render only when that shape is selected, since the flat shape no longer uses them at all).

**Standing workflow, saved to session memory**: the user will paste WhatsFly's real dashboard-generated example every time a new template is created, specifically so this contract gets updated from evidence — a template with a different header type (video/document) or a buttons component should get the same treatment (ask for/wait for its real example) rather than extrapolating from the image-header pattern above.
