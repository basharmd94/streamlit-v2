...
Single-message test panel, calling Meta's own WhatsApp Cloud API directly (`graph.facebook.com`) — send one message (plain text or an approved template) to one hand-entered number, see the raw response. (A prior WhatsFly-routed integration existed alongside this and was removed once Direct WhatsApp was confirmed working end-to-end — no code from it remains.)

- **Credentials**: `config/direct_whatsapp.ini` (gitignored, section `[direct_whatsapp]` with `access_token`/`phone_number_id`/`waba_id`, optional `graph_api_version` defaulting to `v21.0`) via `config/settings.py::get_direct_whatsapp_params()` — returns `None` (never raises) if missing, so the view shows a setup `st.warning` instead of crashing.
- **`core/direct_whatsapp.py`** — thin client: `send_text`, `get_templates`, `send_template`, `upload_media`. Follows Meta's own **published, documented** Cloud API contract directly — no defensive multi-key guessing needed, since the shape isn't reverse-engineered.
- **Exactly one request shape**: `POST /{phone_number_id}/messages` with the nested `template: {name, language: {code}, components: [...]}` body.
- **Two Meta placeholder formats, both supported** — chosen at template-creation time in Meta's own template editor, never mixed within one template: **positional** (`{{1}}`, `{{2}}`) or **named** (`{{cusname}}`, `{{cuscode}}`). `_wa_extract_variable_tokens` returns the raw `{{...}}` tokens in order — digit strings for positional, names for named — and `is_named_format` (first token non-numeric) decides how the send payload is built: positional sends plain `{"type": "text", "text": v}` parameters matched by array order; named sends `{"type": "text", "parameter_name": tok, "text": v}` per parameter, matched by name. No free-form naming UI — for a named template the parameter name comes straight from the template body's own token, since Meta ties the name to the approved template itself, not to metadata chosen at send time.
- **Beautified preview + live variable substitution**, via shared helpers in `views/marketing.py` (`_wa_extract_components`, `_wa_format_whatsapp_markup`, `_wa_render_bubble`, `_wa_substitute_preview`, `_wa_extract_variable_tokens`) — plain WhatsApp-template markup/preview rendering, not tied to any one provider. `_wa_format_whatsapp_markup` converts `*bold*`/`_italic_`/`~strike~` to HTML **after** `html.escape()`-ing the source (verified this neutralizes an injected `<script>` tag rather than rendering it). `_wa_substitute_preview` re-renders the body live as `{{token}}` variable inputs are filled in, matching by token text (not by casting to int) so it works for both placeholder formats.
...
- **Account-wide, not per-ZID** — one WhatsApp Business number across the whole group, so `_show_direct_whatsapp_messaging()` takes no `zid` argument.
