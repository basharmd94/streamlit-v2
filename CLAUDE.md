...
### Manual reconciliation — "🔄 Check WhatsFly Directly" (campaign detail, admin-only)

Follow-up to the staleness incident above: the user checked WhatsFly's own dashboard directly and confirmed the 37 messages genuinely sent, with real status (some read, one delivery failure) already known **on WhatsFly's side** — their webhook just never called this server to report it. So the ground truth exists, just not in our DB, and there was no way to pull it.

**`core/whatsfly.py::get_message_status(wa_message_id)`** — `GET/POST /whatsapp/get/message-status` (params `apiToken`, `wa_message_id`, `whatsapp_bot_id`), documented in `Whatsfly_Integration_docs/whatsfly-integration-guide.md` with its own explicit note: *"for real-time tracking at scale, prefer the webhook over polling this"* — i.e. it's meant as exactly this fallback, not a webhook replacement. `whatsapp_bot_id` needs no new credential — confirmed it's the same value already configured as `phone_number_id` in `config/whatsfly.ini`. **Response shape not yet confirmed against the live account** (same exploratory stance `get_templates`/`upload_media` started with) — raw JSON returned as-is; the plan is the user runs it once against a real unconfirmed wamid and pastes the response back so parsing can be tightened, same evidence-based workflow as every other WhatsFly contract in this app (see `[[whatsfly-template-contract-workflow]]` session memory).

**UI**: in Campaign History's per-campaign detail (`_show_wf_campaign_history`), a new `recipients["webhook_confirmed"]` boolean — **distinct from `delivery_status == "sent"`**, which the existing `.fillna(recipients["status"])` can ALSO produce when the webhook genuinely reported a real `"sent"` status event; `webhook_confirmed` instead checks directly whether `get_current_status_by_wamids` returned ANY row at all for that wamid, which is the precise "never got a webhook response, of any kind" signal. When `webhook_confirmed == False` recipients exist for the selected campaign AND `user_role == "admin"`, a **"🔄 Check WhatsFly Directly"** section appears below the recipient table/download button: pick one recipient from a selectbox, click to pull their status straight from WhatsFly via `get_message_status`, shown raw via `st.json`. **Deliberately manual, one-recipient-at-a-time, never automatic** — this is a real live call against the paid WhatsFly account, not something to fire off for 37 recipients without asking. Admin-gated and raw-JSON, matching this app's existing convention for any WhatsFly response whose shape isn't confirmed yet (e.g. the `⚙️ Advanced / Debug` raw template dump in WhatsFly Messaging).

Verified against the real restored production data: for campaign #1 (the healthy Sept 13 test send, all 5 recipients already webhook-confirmed) the unconfirmed-candidate set is correctly empty — the reconciliation section doesn't appear. For campaign #2 (the incident campaign) all 37 recipients correctly show up as candidates.
