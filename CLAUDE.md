...
Combined 100001 + 100009 stock table in Purchase Analysis, showing **`100001 Stock`**, **`100009 Stock`**, and **`Total Stock`** (renamed from just "Stock") side by side, per-ZID columns to the left of the total.

**`final_items_view` has no `zid=100009` branch of its own** (confirmed via `pg_get_viewdef` — it's a 4-way `UNION ALL` over zid 100001/100000/100005 only). For a 100001 item cross-ZID-linked to a 100009 packaging item (via `caitem.xdrawing`), the view's first branch already sums `stk_100001.stock + stk_100009.stock` into that single 100001 row — so `Analytics("inventory_overview", zid="100009")` (`inv_109` in the code) always returns **zero rows**, not just a zero-stock column. A first attempt at this feature summed `inv_101`/`inv_109`'s own `stock` column per ZID, which silently mislabeled the already-combined total as "100001 Stock" while "100009 Stock" always showed 0 — looked like a real bug report ("100009 stock is coming up as 0, all coming 100001") and was.

Fixed by adding `core/queries.py::get_inventory_zid_stock_split` (registered as `"inventory_zid_stock_split"` in `Analytics`), which recomputes the two halves directly from `imtrn`/`caitem`, **mirroring the view's own `stk_100001`/`stk_100009` subqueries exactly** (same warehouses — `'HMBR -Main Store (4th Floor)'` for 100001; `'Finished Goods Store Packaging'`/`'Raw Material Store Packaging'` for 100009 — same `caitem.xdrawing` join, scoped `WHERE ci.zid = 100001`). `views/purchase.py::_load_inventory_zid_stock_split` loads it (1h TTL cache) and merges it onto `inv_101` by `item_id` before the existing `resolved_code` packcode-consolidation grouping runs. Verified against real Postgres: `stock_100001 + stock_100009` reconciles to `final_items_view.stock` with **zero diff across every 100001 item**, confirmed both for unlinked items (e.g. items with no `xdrawing` match, `stock_100009` correctly 0) and linked ones (e.g. item `1204`: 175 + 8250 = 8425, matching the view's combined value exactly).
