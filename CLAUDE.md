...
- **Two very different confidence levels, captioned explicitly**: for the "Meta Cloud API style" payload shape, the header image slots into a real, documented Meta component (`{"type": "header", "parameters": [{"type": "image", "image": {"id": ...} or {"link": ...}}]}`) — well-grounded. For the "Flat" shape (the one confirmed correct for text-only sends), there's no real example yet of how WhatsFly's own flat contract expects an image header — `media_id`/`media_url` are a guess, added to the flat payload only when present, explicitly captioned as unconfirmed since the one real dashboard example available had no header image at all.
- `header_image_param` prefers the uploaded media's `id` over `link`/manual URL when both are available (an uploaded-and-hosted-by-WhatsFly media id is a stronger signal than an arbitrary external URL, and matches how Meta's own API treats `id` vs `link` as alternatives, not both at once).

**First real test, first real findings**: with a genuine image-header template (`whatsapp_support_contact`), the flat shape (`media_id`+`media_url`, no `media_type`) got `{"status":"0","message":"header: Format mismatch, expected IMAGE, received UNKNOWN"}` — the upload itself worked (real media id + hosted S3 URL came back), but nothing told WhatsFly what *kind* of media it was. **Missed on the first pass, caught on re-reading the guide**: `send/interactive-buttons` and `send/file` both already document a `media_url`/`media_id` **+ `media_type`** trio — the flat shape now always adds `"media_type": "image"` alongside `media_id`/`media_url` when a header image is present, matching this API's own established convention rather than a fresh guess.

The Meta Cloud API style attempt on the same template (real, correctly-nested `template.components: [{type: header, ...}, {type: body, ...}]`, per Meta's actual documented shape) got `{"status":"0","message":"Message template not found."}` instead — WhatsFly's own not-found message, not a Meta Graph API error. Since the flat shape's `template_id` alone (no nesting) previously got *past* this exact error to a real Meta-level rejection, and the Meta-style payload has no flat top-level `template_id` at all (only nested `template.name`), the working theory is: **WhatsFly's send endpoint does its own template lookup keyed on the flat `template_id` field specifically**, regardless of what's nested under `template`/`components` — so the Meta Cloud API style shape may need a flat `template_id` added alongside its nested structure to work at all, not either/or. Not yet tested with the `media_type` fix in place — next real send is the concrete next step, not more guessing.
