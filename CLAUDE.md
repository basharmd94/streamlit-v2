...
**UI changed accordingly**: the per-variable input is now two fields (`Name` + `Value`, not just `Value`) — the name is NOT auto-detectable from anything this account's `get/template/list` response returns (no `components`/`example` fields, per the "body/header/footer text fields... unconfirmed" note above), so it's manual entry, sourced by the user from the template's own definition. `default_payload` for the flat shape is now built as `{"template_id": ..., "templateVariable-<name>-<n>": value, ..., "template_header_media_url": ...}` — nothing else. The Meta Cloud API style shape is kept only as a documented fallback (its `template_name`/`language_code` inputs moved to render only when that shape is selected, since the flat shape no longer uses them at all).
