...
- [x] **New top-level radio "📋 Curated List"**, a hand-picked alternative to the filter-builder audience for when the target list is better chosen by eye: the full customer directory for the ZID (~9,959 real rows for 100001 — no area/salesman/score/etc. filters at all, per explicit ask) with a multiselect to add individual customers onto one saved list per ZID, and a matching multiselect + button to remove any of them. New table `curated_list_contacts` (`whatsapp_webhook/add_curated_list_table.sql`, inline in `schema.sql`; grants in `add_curated_list_grants.sql`, inline in `add_streamlit_campaign_role.sql` — **none of the three run for real yet**) and new module `processing/wf_curated_list.py` (`list_curated`/`add_to_curated`/`remove_from_curated`, same upsert/DELETE conventions as everywhere else). Doesn't feed into a send yet — same "build the list first" scope the filter-builder audience already has; the natural next step is reusing `_render_campaign_confirm_and_send`, same as the Test send panel already does.
- [x] **Real, previously-undiscovered access-control bug found and fixed while writing this table's grants**: `streamlit_campaign_writer` was created with only `SELECT, INSERT, UPDATE` on `contact_opt_outs` — but Phase 5's "↩️ Remove Opt-Out" does a real `DELETE`. This would have failed with a permission error the first time anyone actually removed an opt-out on the live server; Phase 5's own verification never caught it because it ran against a scratch database as the *owning* role, which bypasses grants entirely. Fixed via a standalone `add_contact_opt_outs_delete_grant.sql` (**not yet run for real**) plus the fresh-install script corrected in place.
- [x] **Verified live against the actual restricted role this time**, not the DB owner — created `streamlit_campaign_writer` for real in a scratch database from the (now-fixed) grant scripts, connected as that role, and confirmed: adding 2 real customers to the curated list (`INSERT`) works, the add-multiselect correctly resets after a successful add, removing one (`DELETE`) works, and — the actual point of this pass — opting a customer out and then removing that opt-out (`DELETE`) also now works under the real restricted role, closing the loop on the bug found above.
- [ ] Still open: **run** `add_curated_list_table.sql`, `add_curated_list_grants.sql`, and `add_contact_opt_outs_delete_grant.sql` against the live database.
